...
type S = {
  ...
  mode: Record<string, "code" | "diff">;
  dirty: Record<string, boolean>;
  contents: Record<string, string>;
  diskHash: Record<string, string | undefined>;
  conflict: Conflict;
  message: string | null;
  open: (p: string) => void;
  close: (p: string) => void;
  ...
  setContent: (p: string, c: string, h?: string) => void;
  markDirty: (p: string, d: boolean) => void;
...
export const useEditor = create<S>((set) => ({
  ...
  open: (p) =>
    set((s) => ({
      openFiles: s.openFiles.includes(p) ? s.openFiles : [...s.openFiles, p],
      activeFile: p,
    })),
  close: (p) =>
    set((s) => {
      ...
      return {
        openFiles: of,
        activeFile: s.activeFile === p ? of[of.length - 1] ?? null : s.activeFile,
        mode: restMode,
        dirty: restDirty,
  ...
  setContent: (p, c, h) =>
    set((s) => ({
      contents: { ...s.contents, [p]: c },
      ...(h !== undefined
        ? { diskHash: { ...s.diskHash, [p]: h } }
        : {}),
    })),
  markDirty: (p, d) =>
    set((s) => ({
