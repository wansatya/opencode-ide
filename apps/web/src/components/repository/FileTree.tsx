...
const Node = memo(function Node({ node, depth }: { node: FileNode; depth: number }) {
  ...
  if (node.type === "directory") {
    ...
    return (
      <div>
        <button onClick={() => useRepo.getState().toggle(node.path)} className="w-full flex items-center gap-1 px-1 py-[3px] text-[13px] hover:bg-[#281f18] rounded text-[#ece1d8]" style={{ paddingLeft: 6 + depth * 12 }}>
