...
function badge(s?: string) {
  ...
  return <span className="ml-auto text-[10px] px-1 rounded bg-[#4B4646] border border-[#B7B1B1]/40 text-[#F1ECEC] font-medium">{map[s] ?? s[0]}</span>;
}
...
const Node = memo(function Node({ node, depth, onContextMenu }: { node: FileNode; depth: number; onContextMenu?: (x: number, y: number, dir: string, target: string | null, targetType: "file" | "directory" | null) => void }) {
    ...
    return (
      <div>
        <button
          ...
          className="w-full flex items-center gap-1 px-1 py-[3px] text-[13px] hover:bg-[#2c2c2c] rounded text-[#F1ECEC]" style={{ paddingLeft: 6 + depth * 12 }}>
          {open ? <ChevronDown size={13} className="text-[#B7B1B1]" /> : <ChevronRight size={13} className="text-[#B7B1B1]" />}
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-1.5 px-1 py-[3px] text-[13px] rounded ${active ? "bg-[#4B4646] text-[#F1ECEC] font-medium" : "hover:bg-[#2c2c2c] text-[#B7B1B1]"} ${deleted ? "line-through opacity-60" : ""}`}
